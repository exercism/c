...
void test_encode_with_one_rail(void) {
  // TEST_IGNORE();
  test_expected_value("One rail, only one rail", encode("One rail, only one rail", 1));
}
...
void test_encode_with_three_rails(void) {
  // TEST_IGNORE();
  test_expected_value("WECRLTEERDSOEEFEAOCAIVDEN", encode("WEAREDISCOVEREDFLEEATONCE", 3));
}
...
void test_encode_with_less_letters_than_rails(void) {
  // TEST_IGNORE();
  test_expected_value("More rails than letters", encode("More rails than letters", 24));
}
...
void test_decode_with_three_rails(void) {
  // TEST_IGNORE();
  test_expected_value("THEDEVILISINTHEDETAILS", decode("TEITELHDVLSNHDTISEIIEA", 3));
}

void test_decode_with_four_rails(void) {
  // TEST_IGNORE();
  test_expected_value("THEREISNOGOVERNORANYWHERE", decode("TSENEHINVRAYREEOONRWERGOH", 4));
}
