...
void pattern(char *string, size_t len) {
  for (size_t i = 0; i < len; i++) {
    if (string[i]) putchar('*'); else putchar('-');
  }
  putchar('\n');
}
...
char *encode(char *text, size_t rails) {
  ...
  for (size_t rail = 0; rail < rails; rail++) {
    for (size_t i = 0; i < length; i += cell_size) {
      if (i+rail < length) ciphertext[j++] = text[i+rail];
      if (rail > 0 && rail < rails-1) {
        if (i+rails+rail < length) ciphertext[j++] = text[i+cell_size-rail];
      }
    }
  }
  return ciphertext;
...
char *decode(char *ciphertext, size_t rails) {
  ...
  for (size_t rail = 1; rail < rails-1; rail++) {
    for (size_t i = 0; i < length; i += cell_size) {
      if (i+rail < length) text[i+rail] = ciphertext[j++];
      if (i+cell_size-rail < length) text[i+cell_size-rail] = ciphertext[j++];
    }
  }
  for (size_t i = 0; i < length; i += cell_size) {
    if (i+rails-1 < length) text[i+rails-1] = ciphertext[j++];
  }
  return text;
}
