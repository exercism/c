...
static void check_list_contents(list_element_t expected[],
                                list_element_t actual[], size_t expected_length)
{
   TEST_ASSERT_EQUAL_MEMORY_ARRAY(expected, actual,
                                  sizeof(list_element_t), expected_length);
}
...
static void check_lists_match(list_t * expected, list_t * actual)
{
   ...
   // check elements match in non-zero length lists
   if (expected->length) {
      char *error_message = create_error_message(expected, actual);
      TEST_ASSERT_EQUAL_MEMORY_ARRAY_MESSAGE(expected->elements,
                                             actual->elements,
...
static void test_append_list_to_empty_list(void)
{
   ...
   list_t *expected = new_list(expected_len, expected_elements);

   list_t *actual = append_list(list1, list2);
   check_lists_match(expected, actual);
   check_list_contents(expected_elements, actual->elements, expected_len);
