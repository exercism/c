...
static int getDayOfWeekFromDate(unsigned int year, unsigned int month, unsigned int day)
{
...
static int getWeekOfMonth(const char *week)
{
   ...
   return getNumberGivenText(week, &whichWeekOfMonthLookup[0], sizeof(whichWeekOfMonthLookup) / sizeof(TextToIntLookup_t));
}
...
static int getRequestedDayOfWeek(const char *dayOfWeek)
{
   ...
   return getNumberGivenText(dayOfWeek, &dayLookup[0], sizeof(dayLookup)/sizeof(TextToIntLookup_t));
}
