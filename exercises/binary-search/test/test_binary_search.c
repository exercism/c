...
void test_single_element(void)
{
   ...
   TEST_ASSERT_EQUAL_UINT64(0, binary_search(6, arr, length));
}
...
void test_find_value_with_odd_length(void)
{
   ...
   size_t length = sizeof(arr) / sizeof(arr[0]);
   TEST_ASSERT_EQUAL_UINT64(9, binary_search(144, arr, length));
}
...
void test_find_value_with_even_length(void)
{
   ...
   size_t length = sizeof(arr) / sizeof(arr[0]);
   TEST_ASSERT_EQUAL_UINT64(5, binary_search(21, arr, length));
}
...
void test_empty_array(void)
{
   int *arr = NULL;
   size_t length = 0;
   TEST_ASSERT_EQUAL_UINT64(length, binary_search(1, arr, length));
}
